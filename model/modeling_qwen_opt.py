...
from .visual_opt import VisionTransformer
...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.q_proj = nn.Linear(config.hidden_size, self.projection_size)
        self.k_proj = nn.Linear(config.hidden_size, self.projection_size)
        self.v_proj = nn.Linear(config.hidden_size, self.projection_size)
    ...
    def forward(
        ...
        if rotary_pos_emb is not None:
            ...
            rotary_matrix = (
                build_rotary_matrix(rotary_pos_emb[0], rotary_pos_emb[1])
                .unsqueeze_(0)
                .to(query)
            )

            query = query @ rotary_matrix
            key = key @ rotary_matrix
...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        attention_mask = self._prepare_decoder_attention_mask(
            ...
            min=torch.finfo(inputs_embeds.dtype).min,
            max=torch.finfo(inputs_embeds.dtype).max,
        )
        ...
        if fake_images is not None:
            ...
            for idx, (i, a, b) in enumerate(img_pos):
                hidden_states[i][a + 1 : b] = images[idx].to(hidden_states)
                # hidden_states[i] = torch.cat((hidden_states[i, a+1:b], hidden_states[i, :a+1], hidden_states[i, b:]), 0)

                # rotary_pos_emb[0] = torch.cat((rotary_pos_emb[0][:, a+1:b], rotary_pos_emb[0][:, :a+1], rotary_pos_emb[0][:, b:]), 1)
                # rotary_pos_emb[1] = torch.cat((rotary_pos_emb[1][:, a+1:b], rotary_pos_emb[1][:, :a+1], rotary_pos_emb[1][:, b:]), 1)

        # hidden_states1 = torch.cat((hidden_states[:, a+1:b], hidden_states[:, :a+1], hidden_states[:, b:]), 1)
        # rotary_pos_emb1 = list()
        # rotary_pos_emb1.append(torch.cat((rotary_pos_emb[0][:, a+1:b], rotary_pos_emb[0][:, :a+1], rotary_pos_emb[0][:, b:]), 1))
        # rotary_pos_emb1.append(torch.cat((rotary_pos_emb[1][:, a+1:b], rotary_pos_emb[1][:, :a+1], rotary_pos_emb[1][:, b:]), 1))
        # hidden_states = hidden_states1
        # rotary_pos_emb = rotary_pos_emb1
