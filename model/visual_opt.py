...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, bias=True) -> None:
        super().__init__()
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        attn = attn @ v
        attn = attn.permute(1, 0, 2).reshape(tgt_len, bsz, self.embed_dim)
...
class Resampler(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        out = self.attn(
            ...
            x + self.pos_embed_kv.unsqueeze(1),
            x,
            attn_mask=attn_mask,
        )
...
class VisualAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        sk = sq
        query_layer = self.q_proj(query)
        key_layer = self.k_proj(query)
        value_layer = self.v_proj(query)
...
class TransformerBlock(nn.Module):
    ...
    def get_cast_dtype(self) -> torch.dtype:
        return self.resblocks[1].mlp.c_fc.weight.dtype

    def get_cast_device(self) -> torch.device:
        return self.resblocks[1].mlp.c_fc.weight.device
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.positional_embedding = nn.Parameter(
            get_abs_pos(
                scale * torch.randn(256, width), self.grid_size[0] * self.grid_size[1]
            )
        )
        ...
        self.attn_pool = Resampler(
            ...
            kv_dim=width,
            norm_layer=norm_layer,
            tgz_size=self.grid_size[0],
        )
        self.ln_post = norm_layer(output_dim)
        self.proj_fc = nn.Linear(output_dim, output_dim, bias=True)

        self.fc_sub_mean = nn.Linear(width, width, bias=False)
