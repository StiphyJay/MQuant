...
class TransformerLayer(nn.Module):
    def __init__(self, config):
        ...
        self.mlp_identity = nn.Linear(
            config.hidden_size, config.hidden_size, bias=False
        )
        self.attn_identity = nn.Linear(
            config.hidden_size, config.hidden_size, bias=False
        )
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = self.attn_identity(attention_input) + attention_output
        ...
        output = self.mlp_identity(mlp_input) + mlp_output
        return output
